	...
	async showusersettings() {
		...
		}
		{
		...
		}
			{
				...
				const fetchCommitMeta = async (): Promise<{short?: string; committedAt?: string; message?: string} | null> =>
					await new Promise<{short?: string; committedAt?: string; message?: string} | null>((resolve) => {
						const xhr = new XMLHttpRequest();
						xhr.open("GET", "/version.json?_=" + Date.now() + "&r=" + Math.random());
						xhr.setRequestHeader("Cache-Control", "no-cache, no-store, must-revalidate");
						xhr.setRequestHeader("Pragma", "no-cache");
						xhr.onload = () => {
			...
			const checkBtn = update.addButtonInput("", I18n.localuser.CheckUpdate(), async () => {
				...
				const origText = btn?.textContent || "";
					...
					try {
						...
						await refreshUpdateStatus(serverVer);
			...
			const versionLine = update.addText("");
			...
			const latestCommitLine = update.addText("");
			const commitTimeLine = update.addText("");
			const commitMessageLine = update.addText("");
			const setApplyVisible = (visible: boolean) => {
				const row = applyBtn.buttonHtml?.parentElement;
				if (!row) return;
				row.hidden = !visible;
				if (!visible && applyBtn.buttonHtml) {
					applyBtn.buttonHtml.disabled = false;
					applyBtn.buttonHtml.textContent = "刷新以应用更新 🔄";
				}
			};
			const refreshUpdateStatus = async (serverVerHint?: string) => {
				const [commitMeta, fetchedServerVer] = await Promise.all([
					fetchCommitMeta(),
					serverVerHint ? Promise.resolve(serverVerHint) : fetchLatestVersion(),
				]);
				const serverVer = (commitMeta?.short || "").trim().substring(0, 8) || fetchedServerVer;
				const needsUpdate = serverVer !== "?" && localVer !== serverVer;
				const match = needsUpdate ? "⚠️ 需更新" : "✅";
				versionLine.setText(`运行: ${localVer} | 最新: ${serverVer} ${match}`);
				setApplyVisible(needsUpdate);
				if (!commitMeta) {
					latestCommitLine.setText("");
					commitTimeLine.setText("");
					commitMessageLine.setText("");
					return;
				}
				const when = commitMeta.committedAt ? new Date(commitMeta.committedAt).toLocaleString() : "?";
				latestCommitLine.setText(`最新提交: ${(commitMeta.short || serverVer || "?").substring(0, 8)}`);
				commitTimeLine.setText(`提交时间: ${when}`);
				commitMessageLine.setText(commitMeta.message ? `提交信息: ${commitMeta.message.trim()}` : "");
			};
